...
class DDL_Conveter(object):
    ...
    def add_ukeys(self, keys):
        self.xml_parser.parse(self.src_xml_file)
        query = self.xml_parser.get_add_ukey_ddl(keys)
        self.db_manager.alter_column(query)
    ...
    def remove_ukeys(self):
        ...
        if ukeys:
            for key in ukeys:
                query = self.xml_parser.get_rm_ukey_ddl(key)
                self.db_manager.alter_column(query)
    ...
    def cmp_xml_files(self, file):
        ...
        src_ukeys = self.xml_parser.get_ukey_names()
        ...
        target_ukeys = self.xml_parser.get_ukey_names()
